...
public interface Knife {
	 ...
	 *            当{@link KnifeBox}根据字符的性质(是否为{@link #ASSIGNED}
	 *            )选择Knife分解时，index=offset。
	 * @return index位置的字符在本Knife中的性质规定 <br>
	 *         当offset==index时，仅当返回ASSIGNED时，该Knife才有机会被{@link KnifeBox}
	 *         分配接收文本内容进行分词<br>
	 *         (即才有机会调用dissect方法)
	 * @see #LIMIT
	 * @see #ASSIGNED
	 ...
	 * @param beef
	 *            待分解的字符串内容，这个字符串可能是所要分解的全部字符串的一部分(比如文章中的某一部分)，当beef的最后一个字符为'\0'
	 *            时，表示此次分解是文章最后一段。
	 * @param offset
	 ...
	 *         如果，当其==offset时，表示{@link KnifeBox}
	 *         应该继续遍历还未遍历的Knife，确定是否有其他Knife接收分解offset位置开始的文本内容<br>
	 *         <p>
	 *         负数时：该负数的绝对值必须>=offset。这个绝对值表示此次成功分解了从offset到该绝对值的文本流，剩下的字符，
	 *         该knife已经不能正确解析。(一般此时应该重新传入新的beef对象解析)
	 *         <p>
