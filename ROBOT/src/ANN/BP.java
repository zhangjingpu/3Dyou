...
public class BP implements Serializable {
	...
	public BP(int inputSize, int hiddenSize, int outputSize, double eta, double momentum) {
		...
		// The random's value in the first is 19881211
	...
	/**
	 * Constructor with default eta = 0.05 and momentum = 0.2.
	 * 
	 * @param inputSize
	 * @param hiddenSize
	 * @param outputSize
	...
	public BP(int inputSize, int hiddenSize, int outputSize) {
		this(inputSize, hiddenSize, outputSize, 0.3, 0.5);
	}
	 ...
	 * @param arg
	 */
	private void loadTarget(double[] targe) {
		if (targe.length != target.length - 1) {
			throw new IllegalArgumentException("Size Do Not Match.");
		}
		System.arraycopy(targe, 0, target, 1, targe.length);
	}
	...
	private void adjustWeightHidden() {
		hidden[0] = 1;
		for (int i = 1, len = output.length; i != len; ++i) {
			for (int j = 0, len2 = hidden.length; j != len2; ++j) {
				double newVal = momentum * hidOptPrevUptWeights[j][i] + eta * optDelta[i] * hidden[j];
				hidOptWeights[j][i] += eta * (target[i] - output[i]) * output[i] * (1 - output[i]) * hidden[j];
				hidOptPrevUptWeights[j][i] = newVal;
			}
		}
	}
